...
import { ReadLocalStorageUser } from '../../services/LocalStorageTokenService';

function Panel() {

    const { name } = useParams();
    const mlocalizer = dayjsLocalizer(dayjs)
    const currentUser = ReadLocalStorageUser();

    if (name !== currentUser?.name) {
        return <Navigate to="/auth" replace />; // Przekierowanie na stronę logowania
    }
    return (
      <div className="flex-1 h-screen space-y-6 overflow-y-auto p-6 lg:ml-1/6">
          <div className="flex items-center justify-between">
                <h2 className="text-2xl font-bold">Welcome { name } 👋</h2>
              <button className="rounded bg-dark-blue px-4 py-2 text-xl font-bold text-white">Add New Event</button>
          </div>
